...
class DatabaseManger {
  ...
  Future<Excercise> selectExcercise(int id) async {
    final maps = await select(id, ExcerciseDatabaseSetup.tableName,ExcerciseDatabaseSetup.id,ExcerciseDatabaseSetup.valuesToRead);

    return Excercise.fromJson(maps);
  }
  Future<BodyPart> selectBodyPart(int id) async {
    final maps = await select(id, BodyPartDatabaseSetup.tableName,BodyPartDatabaseSetup.id,BodyPartDatabaseSetup.valuesToRead);

    return BodyPart.fromJson(maps);
  }

  Future<Map<String, Object?>> select(int id, String tableName, String idName, List<String> valuesToRead) async
  {
    final db = await instance.database;
  ...
  Future<List<Excercise>> selectAllExcercise() async {
    final db = await instance.database;
    return (await db.query(ExcerciseDatabaseSetup.tableName, orderBy: "${ExcerciseDatabaseSetup.name} ASC")).map((json) => Excercise.fromJson(json)).toList();
  }
  Future<List<BodyPart>> selectAllBodyParts() async {
    final db = await instance.database;
    return (await db.query(BodyPartDatabaseSetup.tableName, orderBy: "${BodyPartDatabaseSetup.name} ASC")).map((json) => BodyPart.fromJson(json)).toList();
  }
...
//Should be delete implemented for BodyPart?
  ...
  Future initialData() async {
    ...
    await db.insert(Excercise(name:"Podciąganie",description: "pod chwytem tylko",category:  1));
    await db.insert(Excercise(name:"Szruksy",description: "czuje ze zyje",category: 1));
    await db.insert(Excercise(name:"Modlitewnik",description: "+",category:  5));
  }
